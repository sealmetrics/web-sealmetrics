...
| `/for/hotels` a la barra de 1.800 palabras (EN+ES) | **Hecha** — plantilla v4, ~2.200 palabras; recorrido de reserva contrastado con la doc (passthrough referrers por API y por cuenta, un dominio y sus subdominios por sitio, propiedades de conversión); fuera los "webhooks" de PMS, el "defendible bajo RGPD" categórico, el schema con "100%" y el "25% de reservas" sin fuente del hub `/for` |
| `/es/blog/cookieless-analytics-for-hotels` | **Hecha** — ES nativo (~2.000 palabras) con FAQ visible y hreflang; el EN corregido a la vez: fuera los "webhooks", el 65–75% móvil y los ciclos de 14–30 días sin fuente, el "gap del 40–60%" y los rollups de portfolio sin respaldo; ITP descrito como en el glosario. Nuevo campo `dateEs` en `blog.ts` para que un ES publicado después no herede la fecha del EN |
| `/for/agencies` | **Hecha** (15 sep) — plantilla v4 (EN+ES), contrastada con `platform/organizations` (Owner/Admin/Member, un login en varias organizaciones, selector), `channel-grouping` (reglas por sitio, exportación CSV e importación que sustituye, 100 por sitio), `api-tokens` (solo lectura, con permisos y caducidad) y `mcp-server` (un sitio por consulta). Ángulo: la agencia se pone nota a sí misma; el cliente es dueño de la organización y la agencia la opera. Datos: Incapto +37–52% de pago frente a +11% directo, 50% vs 62%; Palladium 40% y +165%; Dreamplace 15–20%. **Fuera hasta que Rafa lo confirme**: acuerdos de reseller con margen, white-label y onboarding en 15 minutos. Eliminado el bloque `agencies` de `VerticalsData.tsx`, que los afirmaba |
...
| 3 | `/blog/meta-ads-conversions-vs-crm` (EN+ES) | **Hecha** — deduplicación píxel + CAPI contrastada con Meta for Developers (event_id + event_name, 48 h); configuración de atribución de Meta solo en términos generales (el Help Center no se pudo leer y los valores por defecto cambiaron en 2026); tres columnas (Meta, medido en web desde tráfico de Meta, CRM), tabla de lectura de diferencias; sin prometer deduplicación ni matching de usuario (Sealmetrics no envía conversiones a Meta, no identifica usuarios ni guarda IDs de pedido). Dreamplace como caso. Prompt 16 en §9b |
| 4 | `/blog/consent-mode-measured-vs-modelled` (EN+ES) | **Hecha** — modos básico y avanzado y contenido de los pings contrastados con Google Tag Platform; requisitos, métricas modeladas y funciones sin dato modelado (BigQuery, audiencias, retención) con Analytics Help 11161109; Blended vs Observed con 10976610; modelado de Google Ads con 10548233. Cita y cifras de Incapto. Pendiente aparte: ampliar `/glossary/consent-mode-v2` y crear su ES. Prompt 17 en §9b |
| 5 | `/blog/last-click-vs-modelled-attribution` (EN+ES) | **Hecha** — post nuevo en lugar de fusionar: el de 748 palabras responde a otra consulta (por qué el multi-touch falla sin datos completos) y ahora enlaza al nuevo. Modelos de GA4 y funcionamiento del basado en datos con Analytics Help 10596866/16291112; Meridian; modelo de Sealmetrics con la doc de attribution-model. Honesto con lo que el modelado aporta; tabla de decisión. Corregido en el post antiguo: "every touchpoint in every journey is visible" y "100% of traffic" contradecían su propia conclusión y los claims prohibidos. Prompt 18 en §9b |
| 6 | `/blog/ga4-vs-piwik-pro-vs-sealmetrics` (EN+ES) | **Hecha** — tabla a tres con datos fechados de cada fabricante (precios y regiones de Piwik PRO, modos de identificadores y su coste según su help center, MCP beta; retención, exportación a BigQuery y MCP experimental de GA4); pesos de tracker del 27 ago; sin porcentaje inventado para Piwik PRO (no hay medición en paralelo); cuándo gana cada uno. Enlaza a `/vs-ga4` y `/vs/piwik-pro` sin competir con ellas. Detectado de paso: el capítulo de precios de `/open` dice "Business from €35/mo · Enterprise quote-only" (hoy 36 € y desde 366 €/mes) y la tarjeta de Dreamplace en `/vs/piwik-pro` afirma que venía de "un stack EU con cookies" y que cuadró con el PMS "la primera semana", sin respaldo en el caso. Prompt 19 en §9b |
...
| 2 | `/integrations/bigquery` (EN+ES) | **Hecha** — contrastada con `platform/settings/integrations/bigquery`: cuenta de servicio y roles, subida de clave, ubicación UE/EE. UU., frecuencia y backfill, siete tipos de datos y sus tablas, columnas de las dos tablas documentadas, cuatro consultas SQL copiadas tal cual, retraso de 1-2 h, borrar la integración no borra datos. Comparación con la exportación de GA4 (1M eventos/día; sin dato modelado) con la ayuda de Google. No se repiten los precios de GCP de la doc (pueden estar desfasados). La plantilla gana un bloque opcional `examples` con código. El hub `/integrations` enlaza ahora BigQuery, Shopify, WooCommerce y Magento. Resuelto el 15 sep: Rafa confirma que BigQuery está incluido en Agentic; corregido en toda la web. La doc (`platform/settings/integrations/bigquery`) sigue diciendo Growth, Scale o Enterprise y hay que actualizarla en su repositorio. Prompt 22 en §9b |
| 3 | `/integrations/google-ads` (EN+ES) | **Hecha** (15 sep) — la limitación va arriba, en el hero y la respuesta: no importa conversiones, no alimenta Smart Bidding, no guarda inversión. Plantillas ValueTrack copiadas de `how-to-track-google-ads-campaigns`; gclid como pago y sin doble entrada al recargar (`attribution-model`); comprobado en la ayuda de Google Ads que los parámetros tras {lpurl} llegan a la página de llegada con seguimiento paralelo; modelado de conversiones (700 clics/7 días). No se afirma clasificación de bots (Agent Analytics no está activo) ni un MCP oficial de Google Ads. El hub `/integrations` gana el grupo "Ad platforms". Prompt 23 en §9b |
...
| 2 | DTC reenfocado: `/for/ecommerce` con Incapto (EN+ES) | **Hecha** (15 sep) — plantilla v4. Ángulo: la tienda sabe qué se vendió, no quién lo vendió; las cuatro preguntas de cada semana (canal, campaña, embudo, producto) con la tienda como punto de conciliación. No repite las páginas de plataforma ni el método de `/use-cases/revenue-attribution`. Contrastada con la doc: embudo fijo (Entrances → View Product → Add to Cart → Begin Checkout → Purchase) con tabla por UTM y selector de país, sin embudos propios; artículos de compra en Propiedades → Conv. Items; instalación de 5 a 30 min según plataforma. Cifras solo de Incapto, sin mezclar sus dos ventanas de medición. Fuera: el "340 vs 180" sin fuente, "Black Friday a resolución completa", "0 muestreo", "la verdad de atribución" y los "5 minutos". Eliminado el bloque `ecommerce` de `VerticalsData.tsx`. Cierra la Fase 6. Prompt 26 en §9b |
